                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            ...
                            <small class="text-muted">Modify the role name and configure its permissions matrix.</small>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body px-4 pt-0 pb-4">
                    <div class="mb-4">
                        ...
                        <div class="input-group" style="max-width: 500px;">
                            ...
                            <input type="text" class="form-control border-start-0 ps-0" id="add_name" name="name" required value="{{ old('name', $role->name) }}" placeholder="e.g. Manager">
                        </div>
                    </div>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                @if(isset($actions['view']))
                                                    <div class="d-inline-block">
                                                        <input class="form-check-input permission-checkbox" type="checkbox" name="permissions[]" value="{{ $actions['view']->name }}" id="perm_{{ $actions['view']->id }}" {{ in_array($actions['view']->name, $rolePermissions) ? 'checked' : '' }}>
                                                    </div>
                                                @endif
                                            </td>
                                            ...
                                            <td class="text-center">
                                                @if(isset($actions['add']))
                                                    <div class="d-inline-block">
                                                        <input class="form-check-input permission-checkbox" type="checkbox" name="permissions[]" value="{{ $actions['add']->name }}" id="perm_{{ $actions['add']->id }}" {{ in_array($actions['add']->name, $rolePermissions) ? 'checked' : '' }}>
                                                    </div>
                                                @endif
                                            </td>
                                            ...
                                            <td class="text-center">
                                                @if(isset($actions['edit']))
                                                    <div class="d-inline-block">
                                                        <input class="form-check-input permission-checkbox" type="checkbox" name="permissions[]" value="{{ $actions['edit']->name }}" id="perm_{{ $actions['edit']->id }}" {{ in_array($actions['edit']->name, $rolePermissions) ? 'checked' : '' }}>
                                                    </div>
                                                @endif
                                            </td>
                                            ...
                                            <td class="text-center">
                                                @if(isset($actions['delete']))
                                                    <div class="d-inline-block">
                                                        <input class="form-check-input permission-checkbox" type="checkbox" name="permissions[]" value="{{ $actions['delete']->name }}" id="perm_{{ $actions['delete']->id }}" {{ in_array($actions['delete']->name, $rolePermissions) ? 'checked' : '' }}>
                                                    </div>
                                                @endif
                                            </td>
                                                    ...
                                                    <div class="d-flex flex-wrap gap-3">
                                                        @foreach($actions['other'] as $otherPerm)
                                                            <div class="form-check mb-0">
                                                                <input class="form-check-input permission-checkbox" type="checkbox" name="permissions[]" value="{{ $otherPerm->name }}" id="perm_{{ $otherPerm->id }}" {{ in_array($otherPerm->name, $rolePermissions) ? 'checked' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (selectAllCb) {
            ...
            // Trigger sync on page load to pre-check "Select All" if all loaded check boxes are check
            syncSelectAllState();
        }
    });
</script>
